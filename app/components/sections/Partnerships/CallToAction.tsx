...
import { HeroSection, HeroContent, BaseSection } from "@/lib/db";
import {
  EditableText,
  EditableTextarea,
  EditableColorPicker,
...
interface CallToActionContent extends HeroContent {
...
export default function CallToAction({
  ...
  const [localContent, setLocalContent] = useState<CallToActionContent>({
    ...defaultContent,
    ...content,
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      if (!document.head.querySelector("style[data-cta]")) {
        const style = document.createElement("style");
        style.setAttribute("data-cta", "true");
        style.innerHTML = `
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="space-y-4 bg-white p-6 rounded-lg shadow-sm border-l-4" style={{ borderLeftColor: "#f59e0b" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-semibold text-gray-700 text-lg">Appearance & Animation</h4>
          </div>
          <EditableColorPicker
            label="Accent Color"
