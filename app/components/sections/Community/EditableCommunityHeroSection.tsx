...
import { EditableText, EditableTextarea, EditableColorPicker, EditableRange, EditableCheckbox } from "@/app/components/EditableInputs";

interface CommunityHeroContent {
  title?: string;
  subtitle?: string;
  description?: string;
...
type CommunitySectionType = BaseSection<CommunityHeroContent>;

interface Props {
  section: CommunitySectionType;
  isEditing: boolean;
  onUpdate: (updates: Partial<CommunitySectionType>) => void;
}

export default function EditableCommunityHeroSection({
  section,
  isEditing,
  onUpdate,
}: Props) {
          ...
          >
            {primaryButtonText && (
              <a
                ...
                className="px-10 py-5 bg-blue-600 text-white rounded-full font-semibold hover:bg-blue-700 hover:shadow-xl transform hover:scale-105 transition-all duration-300"
              >
                {primaryButtonText}
              </a>
            )}
    ...
    >
      ...
      {showFloatingElements && (
        <div className="absolute inset-0 pointer-events-none">
          <motion.div
            className="absolute top-20 -left-20 w-96 h-96 rounded-full blur-3xl bg-blue-600/10"
            animate={{ y: [0, -50, 20, 0] }}
            transition={{ duration: 20, repeat: Infinity }}
          />
          <motion.div
            className="absolute bottom-32 -right-32 w-96 h-96 rounded-full blur-3xl bg-purple-600/10"
            animate={{ y: [0, 40, -30, 0] }}
            ...
            >
              ...
              <div className="space-y-7">
                ...
                <EditableCheckbox
                  ...
                  onChange={(value: any) => handleUpdate({ showFloatingElements: value })}
                />
              </div>
            </motion.div>
          </div>
